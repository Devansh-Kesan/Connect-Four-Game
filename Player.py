...
class AIPlayer:
    ...
    def evaluation_function(self, board,player1,player2):
            ...
            eval_value = diagonallr(board,self.player_number) + diagonalrl(board,self.player_number) + horizontal_evaluation(board,self.player_number) + vertical_evaluation(board,self.player_number) - (diagonallr(board,self.other_player) + diagonalrl(board,self.other_player) + horizontal_evaluation(board,self.other_player) + vertical_evaluation(board,self.other_player))
    ...
    def get_alpha_beta_move(self, board):
        ...
        def max_value(board, alpha, beta, depth):
            if depth == self.LIMIT:
                utility = self.evaluation_function(board,self.player_number,self.other_player)
                return utility, board

            v = -self.M
            best_board = board
            actions = generate_move(board, self.player_number)
            lst = []
            for a in actions:
        ...
        def min_value(board, alpha, beta, depth):
            ...
            v = self.M
            best_board = None
            actions = generate_move(board, self.other_player)
            for a in actions:
